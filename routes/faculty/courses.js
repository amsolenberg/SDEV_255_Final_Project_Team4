...
router.get('/', async (req, res) => {
  try {
    const courses = await Course.find({})
      .populate('faculty', 'firstName lastName') // Populates faculty details
      .populate('createdBy', 'firstName lastName') // Populates createdBy details
      .lean();
...
router.post('/add', async (req, res) => {
  const { name, code, description, credits, faculty, createdBy } = req.body;
  ...
  try {
    const newCourse = new Course({
      ...
      createdBy: createdBy || req.session.user._id
    });
...
router.post('/delete/:id', async (req, res) => {
  try {
    ...
    // Check if the course was created by the currently logged in user
    ...
    if (studentsWithCourse.length > 0) {
      req.flash('error', 'Cannot delete this course. Students are currently enrolled in it.');
      return res.redirect('/faculty/courses');
    }
